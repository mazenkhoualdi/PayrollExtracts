...
export function classifyDay(row, date) {
  ...
  if (statut === 'absent') return { type: 'absent', label: 'Absent', heures: 0 };
  if (statut === 'conge') return { type: 'conge', label: 'Congé', heures };

  if (statut === 'present') {
    if (type_presence === 'demi_journee') return { type: 'demi', label: 'Demi-journée', heures };
    if (heures >= 16) return { type: 'double', label: 'Journée double', heures };
    if (heures > 8) return { type: 'heures_sup', label: `+${heures - 8}h sup`, heures };
    if (heures === 8) return { type: 'normal', label: 'Journée complète', heures };
    if (heures === 0) return { type: 'autre', label: 'Présent (0h notée)', heures };
...
export function dayAmount(dayType, salaireJournalier, overtimeMult, congePaye) {
  ...
  switch (dayType.type) {
    case 'normal': return salaireJournalier;
    case 'demi': return salaireJournalier * 0.5;
    case 'double': return salaireJournalier * 2;
    case 'heures_sup': {
      const extra = dayType.heures - 8;
      return salaireJournalier + extra * hourly * overtimeMult;
    }
    case 'conge': return congePaye ? salaireJournalier : 0;
 ...
 */
export function buildEmployeeReport(db, emp, start, end, overtimeMult, congePaye) {
  const pointages = queryAll(db,
    'SELECT * FROM pointages WHERE employe_id = ? AND date_pointage >= ? AND date_pointage <= ?',
    [emp.id, start, end]
  );
  const byDate = {};
  pointages.forEach(p => { byDate[p.date_pointage] = p; });

  const avances = queryAll(db,
  ...
  return { emp, days, counts, heuresSupTotal, brut, avances, totalAvances, net };
}
...
export function fmtMoney(n) {
  ...
  ).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 3 }) + ' DT';
}
